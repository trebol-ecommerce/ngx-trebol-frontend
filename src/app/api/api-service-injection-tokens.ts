/*
 * Copyright (c) 2021 The Trébol eCommerce Project
 *
 * This software is released under the MIT License.
 * https://opensource.org/licenses/MIT
 */

/**
 * Service tokens for Angular dependency injection pattern
 */
export const API_SERVICE_INJECTION_TOKENS = {
  access: 'AccessApi',
  accountProfile: 'ProfileAccountApi',
  dataBillingTypes: 'BillingTypesDataApi',
  dataCustomers: 'CustomersDataApi',
  dataImages: 'imagesDataApi',
  dataPeople: 'PeopleDataApi',
  dataProductCategories: 'ProductsCategoriesDataApi',
  dataProductLists: 'ProductListsDataApi',
  dataProductListContents: 'ProductListContentsDataApi',
  dataProducts: 'ProductsDataApi',
  dataSales: 'SalesDataApi',
  dataSalespeople: 'SalespeopleDataApi',
  dataShippers: 'ShippersDataApi',
  dataUserRoles: 'UserRolesDataApi',
  dataUsers: 'UsersDataApi',
  about: 'PublicAboutApi',
  categories: 'PublicCategoriesApi',
  checkout: 'PublicCheckoutApi',
  guest: 'PublicGuestApi',
  login: 'PublicLoginApi',
  products: 'PublicProductsApi',
  receipt: 'PublicReceiptApi',
  register: 'PublicRegisterApi',
};
